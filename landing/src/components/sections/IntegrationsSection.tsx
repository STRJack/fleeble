...
const integrations = [
  {
    name: 'Claude Code',
    desc: 'Approve tool calls, answer questions, and manage permissions — all from Fleeble.',
      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="text-xl max-w-xl mx-auto leading-relaxed" style={{ color: 'rgba(var(--card-text-rgb), 0.5)' }}>
            One-click setup for the AI coding tools you already use every day.
          </p>
        </motion.div>
        ...
        >
          {integrations.map((int) => (
            <motion.div
              key={int.name}
              variants={item}
              className="p-8 rounded-2xl text-center transition-all duration-200 hover:-translate-y-1"
              style={{
            ...
            >
              ...
              <h3 className="text-xl font-bold mb-3" style={{ color: '#f1f0f5' }}>{int.name}</h3>
              <p className="text-sm leading-relaxed" style={{ color: '#a09cb0' }}>{int.desc}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
